...
if($_SERVER['REQUEST_METHOD']=='POST')
{
    ...
    $base64EncodedFileString    = $_POST['file_base_code'];
    ...
    else if(empty($base64EncodedFileString))
    {
        $response['status']     = false;
    ...
    {
        /* decode base64Encoded file */
        $decodedFile        = base64_decode($base64EncodedFileString);
        ...
        Allow extensions
        */
        $fileExtensions     = ['jpeg','jpg','png','gif', 'pdf', 'exe','7z', 'vcf', 'csv'];
        $fileExtension      = convertMIMETypeToExtension($MIME_Type);
        ...
        */
        $fileCustomName             = hash("sha1", basename($fileNameSendByUser)."-".bin2hex(openssl_random_pseudo_bytes(32))).".".$fileExtension;
        ...
        */
        if (in_array($fileExtension,$fileExtensions)) 
        {
            ...
            else
            {
                try 
                {
                    if (!file_put_contents($targetDirectory, $decodedFile))
                    {
                        $response['status']     = false;
